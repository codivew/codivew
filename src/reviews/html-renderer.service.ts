...
import type { ReviewIssue, ReviewResult, ReviewSeverity } from './types/review-result.js';
import type { ReviewRequest } from './types/review-request.js';
import { parseUnifiedDiff, type ParsedDiffFile, type ParsedDiffLine } from './unified-diff.js';
...
export class HtmlRendererService {
  render(context: ReviewRenderContext): string {
    ...
    const title = `Codivew - ${request.repository} - ${context.reviewId}`;
...
${this.row('저장소', request.repository)}${this.optionalRow('기준 브랜치', request.baseBranch)}${this.row('리뷰 모드', mode)}${this.optionalRow('커밋 SHA', request.commitSha)}${this.row('리뷰 ID', context.reviewId)}${this.row('검토 파일', `${filtered.reviewedFiles.length}개`)}
</dl></section></div>
<section class="panel"><h2>리뷰 요약</h2><p>${escapeHtml(result.summary)}</p></section>
${this.issueSections(result.issues, parsedDiff)}
...
<section class="panel"><h2>생성 정보</h2><dl class="meta">${this.row('생성 시각', this.formatDateTime(context.createdAt))}${this.row('처리 시간', `${(context.elapsedMs / 1000).toFixed(1)}초`)}${this.row('모델', context.model)}</dl></section>
</main></body></html>`;
  }

  private issueSections(issues: ReviewIssue[], files: ParsedDiffFile[]): string {
    if (issues.length === 0) {
      return '<section><h2>리뷰 항목</h2><p class="empty">발견된 문제가 없습니다.</p></section>';
    }
    return [
      this.section('필수 수정', 'must_fix', issues, files),
      this.section('수정 권장', 'should_fix', issues, files),
      this.section('제안', 'suggestion', issues, files),
    ].join('');
  }
  ...
  private section(
    ...
    return `<section><h2>${title}</h2>${matches.map((issue) => this.issue(issue, issues.indexOf(issue), files)).join('')}</section>`;
  }
...
${this.row('신뢰도', `${Math.round(issue.confidence * 100)}%`)}${this.row('설명', issue.description)}${this.optionalRow('영향', issue.impact)}${this.optionalRow('수정 제안', issue.suggestion)}
  ...
  private diffFile(file: ParsedDiffFile, fileIndex: number, issues: ReviewIssue[]): string {
    ...
    const open = displayedLineCount <= COLLAPSED_DIFF_LINE_THRESHOLD ? ' open' : '';
    const metadata =
    ...
    return `<details class="diff-file" id="diff-file-${fileIndex}"${open}><summary class="diff-file-header"><code>${escapeHtml(file.path)}</code><span class="muted">변경 ${changedLineCount}줄 · 피드백 ${fileIssues.length}개</span></summary>${metadata}${body}</details>`;
  }
  ...
  private diffLine(
    ...
    return `${row}<tr class="inline-feedback"><td colspan="4">${attached
      .map((issue) => {
        ...
        return `<a class="inline-note" href="#feedback-${issueIndex}"><span class="inline-arrow">↳</span><span class="inline-severity">${severityLabels[issue.severity]}</span><span class="inline-title">${escapeHtml(issue.title)}</span><span class="inline-detail">상세 보기 ↑</span></a>`;
      })
      .join('')}</td></tr>`;
  }
